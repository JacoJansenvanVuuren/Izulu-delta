...
const ClientTable = ({
  ...
  selectedMonth,
  currentYear
}: ClientTableProps): JSX.Element | null => {
  ...
  if (safeInitialClients.length === 0 && !onAddClient) return <div className="flex justify-center items-center h-full">
  ...
  const filteredClients = clients.filter(client => {
    ...
    return client.name.toLowerCase().includes(searchLower) || client.location.toLowerCase().includes(searchLower) || client.policyNumbers.some(num => num.toLowerCase().includes(searchLower)) || client.products.some(product => product.toLowerCase().includes(searchLower)) || client.policyPremium.toLowerCase().includes(searchLower);
  });
  ...
  const autoSaveRow = async (clientId: string) => {
    ...
    try {
      ...
      const updatedClient = {
        ...client,
        ...unsavedChanges[clientId]
      };
      await new Promise<void>((resolve, reject) => {
        onUpdateClient(updatedClient, err => {
  ...
  const handleFileUpload = async (clientId: string, field: 'loaDocUrl', file: File) => {
    ...
    try {
      ...
      const updatedClients = clients.map(c => c.id === clientId ? {
        ...c,
        [field]: url
      } : c);
  ...
  const updateMultiEntryField = (clientId: string, field: 'products' | 'policyNumbers', values: string[]) => {
    const updatedClients = clients.map(client => client.id === clientId ? {
      ...client,
      [field]: values
    } : client);
    setClients(updatedClients);
  ...
  return <div className="space-y-4 relative">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-center gap-4 mb-6">
        <div className="relative w-full sm:w-auto">
          ...
          <Input placeholder="Search clients, policies, or products..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="pl-10 bg-transparent border-white/10 w-full sm:w-80 focus:w-full expandable-input" />
        </div>
        
        <div className="flex items-center space-x-2 w-full sm:w-auto justify-end">
          <Button onClick={addNewClient} className="bg-primary/20 hover:bg-primary/30 text-primary border border-primary/30 w-full sm:w-auto" disabled={actionLoading}>
      ...
      <div className="glass-morphism rounded-lg overflow-hidden hidden sm:block">
        <Table>
          ...
          <TableBody>
            {filteredClients.map(client => <TableRow key={client.id} className="border-t border-white/5 hover:bg-white/5" onClick={() => handleRowClick(client.id)}>
                <TableCell>
                  <Input value={client.name} onChange={e => updateClientField(client.id, 'name', e.target.value)} className="bg-transparent border-white/10 w-full min-w-[150px] expandable-input" />
                </TableCell>
                <TableCell>
                  <Input value={client.location} onChange={e => updateClientField(client.id, 'location', e.target.value)} className="bg-transparent border-white/10 w-full min-w-[120px] expandable-input" />
                </TableCell>
                <TableCell>
                  <Input type="number" value={client.policiesCount} onChange={e => updateClientField(client.id, 'policiesCount', parseInt(e.target.value) || 0)} className="bg-transparent border-white/10" />
                </TableCell>
                <TableCell>
                ...
                <TableCell>
                  <div className="relative flex items-center">
                    <Input id={`issue-date-${client.id}`} type="date" value={client.issueDate} onChange={e => updateClientField(client.id, 'issueDate', e.target.value)} className="bg-transparent border-white/10 pr-10" style={{
                  colorScheme: 'dark'
                }} />
                ...
                <TableCell>
                  <div className="relative flex items-center">
                    <Input id={`deduction-date-${client.id}`} type="date" value={client.deductionDate} onChange={e => updateClientField(client.id, 'deductionDate', e.target.value)} className="bg-transparent border-white/10 pr-10" style={{
                  colorScheme: 'dark'
                }} />
                ...
                <TableCell>
                  <div className="relative">
                    ...
                    <Input value={(client.policyPremium || '').replace(/^[$R]/, '')} onChange={e => {
      ...
      {hasUnsavedChanges && Object.keys(unsavedChanges).length > 0 && <div className="fixed bottom-6 right-6 z-50">
          <Button onClick={saveAllChanges} disabled={actionLoading} className="bg-white text-black hover:bg-gray-200 shadow-lg px-5 py-2 rounded-md transition-all duration-200 opacity-100" style={{
        backdropFilter: 'none',
        background: 'white'
      }}>
